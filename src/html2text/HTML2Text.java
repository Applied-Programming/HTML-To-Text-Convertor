...
public class HTML2Text {
    ...
    public String Convert_HTML2Text(String source) throws Exception {
        ...
        try {
            ...
            while (c != -1) // Convert until EOF
            {
                ...
                if (c == '<') // It's a tag!!
                {
                    String current_tag = get_tag(input); // Get the rest of the tag
                    text = convert_tag(current_tag);
                } else if (c == '&') {
                    String specialchar = get_special(input);
    ...
    String get_tag(Reader r) throws IOException {
        StringBuffer result = new StringBuffer();
        int level = 1;
    ...
    String get_special(Reader r) throws IOException {
        StringBuffer result = new StringBuffer();
    ...
    boolean is_tag(String s1, String s2) {
        s1 = s1.toLowerCase();
    ...
    String convert_tag(String t) throws IOException {
        String result = "";

        if (is_tag(t, "body")) {
            in_body = true;
            body_found = true;
        } else if (is_tag(t, "/body")) {
            in_body = false;
            result = " ";
        } else if (is_tag(t, "center")) {
            result = " ";
            center = true;
        } else if (is_tag(t, "/center")) {
            result = " ";
            center = false;
        } else if (is_tag(t, "pre")) {
            result = " ";
            pre = true;
        } else if (is_tag(t, "/pre")) {
            result = " ";
            ...
            result = " ";
        } else if (is_tag(t, "h1") || is_tag(t, "h2") || is_tag(t, "h3") || is_tag(t, "h4") || is_tag(t, "h5") || is_tag(t, "h6") || is_tag(t, "h7")) {
            result = " ";
        } else if (is_tag(t, "/h1") || is_tag(t, "/h2") || is_tag(t, "/h3") || is_tag(t, "/h4") || is_tag(t, "/h5") || is_tag(t, "/h6") || is_tag(t, "/h7")) {
            result = " ";
        } else if (is_tag(t, "/dl")) {
        ...
        } else if (is_tag(t, "table")) {
            result = " ";
        } else if (is_tag(t, "/table")) {
            result = " ";
        } else if (is_tag(t, "form")) {
            result = " ";
        } else if (is_tag(t, "/form")) {
            result = " ";
        } else if (is_tag(t, "b")) {
    ...
    public static void main(String argv[]) throws Exception {
		...
		//if (argv[0] != null) file = new File(argv[0]);
            ...
            String text = h.Convert_HTML2Text(s);
